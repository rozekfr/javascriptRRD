...
function rrdRRAMultiFlotObj(rrd_files, // a list of [rrd_id,rrd_file] pairs, all rrds must have the same step
    ...
    // reverse order so higher numbers are behind
    for (var id=0; id<tmp_nr_ids; id++) {
        out_el.data.push(tmp_flot_els[id]);
    }
...
rrdFlotMatrix.prototype.drawFlotGraph = function() {
  ...
  var flot_obj=rrdRRAMultiFlotObj(rrd_list,rra_idx,ds_id);
  ...
  for (var i=0; i<flot_obj.data.length; i++) {
	...
	flot_obj.data[i].label=dgo['title'];
      ...
      if (dgo['lines']!=null) {
	// if the user provided the line, use it
	flot_obj.data[i].lines=dgo['lines'];
      }
    }
    if (flot_obj.data[i].lines==null) {
